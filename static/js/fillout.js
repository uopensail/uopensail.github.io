...
`;
    ...
    const getSharedIframeSrc = (configDomain, flowPublicIdentifier, inheritParameters, target) => {
        let domain = 'https://forms.fillout.com';
        if (configDomain) {
    ...
    const initializePopupLikeTarget = (target, embedType) => {
        const { flowPublicIdentifier, initialized, inheritParameters, sliderDirection, domain, } = getConfig(target);
        if (initialized)
            return;
        const popupContainer = document.createElement('div');
        popupContainer.className = `fillout-embed-${embedType}`;
    ...
    const initializeStandardTarget = (target, isFullScreen) => {
        const { initialized, flowPublicIdentifier, inheritParameters, dynamicResize, domain, } = getConfig(target);
        if (initialized)
            return;
        ...
        if (dynamicResize) {
            ...
            const receiveMessage = (event) => {
                if (event.origin !== new URL(iframeSrc.toString()).origin ||
